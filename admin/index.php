...
if($m){
 ...
 elseif(strpos($m,"renew_")===0){$t='ต่ออายุ '.substr($m,6).' สำเร็จ';}
...
<div class="gr"><div class="cd"><h3>สร้างผู้ใช้ SSH</h3><div><input id="c_user" placeholder="ชื่อผู้ใช้"><input id="c_pass" placeholder="รหัสผ่าน"><input id="c_days" type="number" value="30"><input id="c_gb" type="number" value="0" placeholder="กำหนด GB (0=ไม่จำกัด)"><input id="c_ip" type="number" value="0" placeholder="ลิมิต IP (0=ไม่จำกัด)"><button class="btn-p" onclick="var u=document.getElementById('c_user').value,p=document.getElementById('c_pass').value,d=document.getElementById('c_days').value,g=document.getElementById('c_gb').value,i=document.getElementById('c_ip').value;if(u&&p){location.href='?page=create&action=create&user='+u+'&pass='+p+'&days='+d+'&gb='+g+'&ip='+i;}">สร้าง</button></div></div>
